...
type AtfPolicies struct {
	AtfPolicyList []AtfPolicyDetail `json:"atf-policy"` // Air Time Fairness policy list (Live: IOS-XE 17.12.6a)
}
...
type RFProfiles struct {
	RFProfileList []RFProfileDetail `json:"rf-profile"` // Radio-Frequency profile list (Live: IOS-XE 17.12.6a)
}

// RFProfileDefaultEntries represents RF profile default entries collection.
type RFProfileDefaultEntries struct {
	RFProfileDefaultEntryList []RFProfileDefaultEntry `json:"rf-profile-default-entry"` // Default RF profile entry list (Live: IOS-XE 17.12.6a)
}

// MultiBssidProfile represents Multi-BSSID profile configuration.
type MultiBssidProfile struct {
	ProfileName string `json:"profile-name"`          // 802.11ax Multi BSSID profile name (Live: IOS-XE 17.12.6a)
	Description string `json:"description,omitempty"` // Brief Multi BSSID profile description (Live: IOS-XE 17.12.6a)
}

// AtfPolicy represents ATF policy structure container.
type AtfPolicy struct {
	AtfPolicyList []AtfPolicyDetail `json:"atf-policy"` // Air Time Fairness policy objects list (Live: IOS-XE 17.12.6a)
}

// AtfPolicyDetail represents ATF policy configuration details.
type AtfPolicyDetail struct {
	PolicyID      int    `json:"policy-id"`                // Unique ATF policy identifier (Live: IOS-XE 17.12.6a)
	AtfPolicyName string `json:"atfpolicy-name"`           // ATF policy name (Live: IOS-XE 17.12.6a)
	PolicyWeight  int    `json:"policy-weight,omitempty"`  // ATF policy weight percentage (Live: IOS-XE 17.12.6a)
	ClientSharing bool   `json:"client-sharing,omitempty"` // ATF client fair sharing flag (Live: IOS-XE 17.12.6a)
}
...
type RFProfileDetail struct {
	Name                             string                `json:"name"`                                           // RF profile name identifier (Live: IOS-XE 17.12.6a)
	Description                      string                `json:"description,omitempty"`                          // RF profile description (YANG: IOS-XE 17.12.1)
	TxPowerMin                       int                   `json:"tx-power-min,omitempty"`                         // Minimum transmit power in dBm (Live: IOS-XE 17.12.6a)
	TxPowerMax                       int                   `json:"tx-power-max,omitempty"`                         // Maximum transmit power in dBm (YANG: IOS-XE 17.12.1)
	TxPowerV1Threshold               int                   `json:"tx-power-v1-threshold,omitempty"`                // TPC version 1 threshold in dBm (Live: IOS-XE 17.12.6a)
	TxPowerV2Threshold               int                   `json:"tx-power-v2-threshold,omitempty"`                // TPC version 2 threshold in dBm (YANG: IOS-XE 17.12.1)
	Status                           bool                  `json:"status,omitempty"`                               // RF profile operational state (Live: IOS-XE 17.12.6a)
	Band                             string                `json:"band,omitempty"`                                 // Radio frequency band type (Live: IOS-XE 17.12.6a)
	...
	DataRate6M                       string                `json:"data-rate-6m,omitempty"`                         // 6 Mbps data rate state (Live: IOS-XE 17.12.6a)
	DataRate9M                       string                `json:"data-rate-9m,omitempty"`                         // 9 Mbps data rate state (Live: IOS-XE 17.12.6a)
	DataRate12M                      string                `json:"data-rate-12m,omitempty"`                        // 12 Mbps data rate state (Live: IOS-XE 17.12.6a)
	DataRate18M                      string                `json:"data-rate-18m,omitempty"`                        // 18 Mbps data rate state (Live: IOS-XE 17.12.6a)
	DataRate24M                      string                `json:"data-rate-24m,omitempty"`                        // 24 Mbps data rate state (Live: IOS-XE 17.12.6a)
	...
	LoadBalancingWindow              int                   `json:"load-balancing-window,omitempty"`                // Load balancing window seconds (Live: IOS-XE 17.12.6a)
	LoadBalancingDenialCount         int                   `json:"load-balancing-denial-count,omitempty"`          // Load balancing denial count (Live: IOS-XE 17.12.6a)
	...
	RfdcaRemovedChannels             *RfdcaRemovedChannels `json:"rfdca-removed-channels,omitempty"`               // RF DCA removed channels data (Live: IOS-XE 17.12.6a)
	ChannelWidthMax                  string                `json:"channel-width-max,omitempty"`                    // Maximum channel width cap (Live: IOS-XE 17.12.6a)
...
type RfdcaRemovedChannels struct {
	RfdcaRemovedChannel []RfdcaRemovedChannel `json:"rfdca-removed-channel"` // DCA removed channel list (Live: IOS-XE 17.12.6a)
}

// RfdcaRemovedChannel represents DCA removed channel configuration.
type RfdcaRemovedChannel struct {
	Channel int `json:"channel"` // Channel number to remove from DCA (Live: IOS-XE 17.12.6a)
}
