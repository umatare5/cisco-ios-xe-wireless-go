...
// Sample program: Count Access Points
//
// This program connects to a Cisco WNC controller and counts the number of managed access points (APs).
//
// Usage:
//
//	Set the environment variables WNC_CONTROLLER and WNC_ACCESS_TOKEN with appropriate values. Then run the program.
//
// Example:
//
//	WNC_CONTROLLER="<controller-host-or-ip>" WNC_ACCESS_TOKEN="<base64-username:password>" go run ./samples/count_aps
//
// Note:
//
//	WithInsecureSkipVerify(true) is for lab/self-signed cert testing only.
package main
