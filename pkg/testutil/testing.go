...
// lives in an internal package, so a test inside this module asserts it back and passes it to a
// service constructor. The supported route from outside the module is wnc.NewClient against a
// MockServer's URL, reaching an untyped node through the root client's GetData.
