...
// newLISPTestDataCollector creates a new test data collector
func newLISPTestDataCollector() *LISPTestDataCollector {
	return &LISPTestDataCollector{
...
func TestLISPServiceIntegration(t *testing.T) {
	...
	collector := newLISPTestDataCollector()
	client := tests.TestClient(t)
	service := NewService(client)
	ctx := tests.TestContext(t)

	t.Run("Oper", func(t *testing.T) {
		runLISPTestAndCollectData(t, collector, "GetOper", func() (interface{}, error) {
