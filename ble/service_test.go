// Package ble provides Bluetooth Low Energy test functionality for the Cisco Wireless Network Controller API.
package ble
...
// BLETestDataCollector holds test data for BLE service functions
type BLETestDataCollector struct {
	Data map[string]interface{} `json:"ble_test_data"`
}

// newBLETestDataCollector creates a new test data collector
func newBLETestDataCollector() *BLETestDataCollector {
	return &BLETestDataCollector{
		Data: make(map[string]interface{}),
	}
}

func runBLETestAndCollectData(t *testing.T, collector *BLETestDataCollector, testName string, testFunc func() (interface{}, error)) {
	data, err := testFunc()
	if err != nil {
...
func TestBLEServiceStructures(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "BLEOperResponse",
			jsonData: `{
				"Cisco-IOS-XE-wireless-ble-oper:ble-oper-data": {
					"ble-enable": true,
					"beacon-interval": 100
				}
			}`,
...
func TestBLEServiceMethods(t *testing.T) {
	...
	// Table-driven test cases for BLE endpoints
	tests := []struct {
		name       string
...
func TestBLEServiceIntegration(t *testing.T) {
	...
	collector := newBLETestDataCollector()
	client := tests.TestClient(t)
	service := NewService(client)
	ctx := tests.TestContext(t)
	...
	if len(collector.Data) > 0 {
		if err := tests.SaveTestDataToFile("ble_test_data_collected.json", collector.Data); err != nil {
			t.Logf("Warning: Could not save test data: %v", err)
		} else {
			t.Logf("BLE test data saved to test_data/ble_test_data_collected.json")
		}
	}
}
