...
// GetTagValidationError returns appropriate error for tag type
func GetTagValidationError(tagType string) error {
	switch tagType {
	case "site":
		return RequiredParameterError("site tag")
	case "policy":
		return RequiredParameterError("policy tag")
	case "rf":
		return RequiredParameterError("RF tag")
	default:
		return ValidationError("tag type", "unknown")
	}
}
