...
var (
	...
	// testFatalf is a hook for fatal logging to enable coverage without aborting testing.
...
func createTestClient(controller, token string) (*core.Client, error) {
	return createCoreClient(controller, token,
		core.WithTimeout(net.DefaultHTTPTimeoutDuration),
...
//
//	client := testutil.OptionalTestClient(t)
...
// Use a more reliable endpoint that should be available on all WNC controllers
var connectivityCheck = func(ctx context.Context, client *core.Client) error {
	// Use general operational data which should be available on all WNC controllers
