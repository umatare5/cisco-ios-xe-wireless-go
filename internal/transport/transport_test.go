...
func TestHTTPConstants(t *testing.T) {
	...
	if DefaultTLSHandshakeTimeout != 5*time.Second {
		t.Errorf("DefaultTLSHandshakeTimeout = %v, want 5s", DefaultTLSHandshakeTimeout)
	}

	if DefaultResponseHeaderTimeout != 5*time.Second {
		t.Errorf("DefaultResponseHeaderTimeout = %v, want 5s", DefaultResponseHeaderTimeout)
	}
