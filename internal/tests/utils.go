...
const (
	...
	EmptyEndpointErrorTemplate    = "Endpoint %s is empty"
	ShortEndpointErrorTemplate    = "Endpoint %s is too short: %s"
	EndpointMismatchErrorTemplate = "Endpoint %s: expected %s, got %s"
)

// HTTP constants - to avoid circular dependency
const (
	HTTPSScheme                = "https"
	HTTPMethodGet              = "GET"
	...
	HTTPHeaderAccept           = "application/yang-data+json"
...
func CreateQuickTestContext() (context.Context, context.CancelFunc) {
	return CreateTestContext(QuickTestTimeout)
}
...
func CreateMicroTestContext() (context.Context, context.CancelFunc) {
	return CreateTestContext(MicroTestTimeout)
}
...
func validateEndpoint(t *testing.T, endpointName, endpointValue string) {
	...
	if endpointValue == "" {
		t.Errorf(EmptyEndpointErrorTemplate, endpointName)
		return
	}

	// Early return for short endpoint
	if len(endpointValue) < MinEndpointLength {
		t.Errorf(ShortEndpointErrorTemplate, endpointName, endpointValue)
		return
	}
}
...
func GenerateEndpointValidationTest(t *testing.T, expectedEndpoints map[string]string, actualEndpoints map[string]string) {
	...
	for name, expected := range expectedEndpoints {
		t.Run(name, func(t *testing.T) {
			...
			if actual != expected {
				t.Errorf(EndpointMismatchErrorTemplate, name, expected, actual)
			}
		})
	}
}
