...
type UrwbCfg struct {
	UrwbCfgData *UrwbCfgData `json:"Cisco-IOS-XE-wireless-urwb-cfg:urwb-cfg-data"`
}
...
// UrwbCfgData represents the URWB configuration data container from YANG 17.18.1+.
...
// UrwbProfile represents URWB policy configuration for AP.
...
// UrwbMob represents URWB mobility configuration.
type UrwbMob struct {
...
// UrwbMpls represents URWB MPLS configuration.
...
// UrwbMpo represents URWB MPO (Multi-Path Optimization) configuration.
type UrwbMpo struct {
...
// EthertypeLists represents URWB ether types list configuration.
type EthertypeLists struct {
	EthertypeList []EthertypeList `json:"ethertype-list"`
}

// EthertypeList represents a single ether type configuration.
type EthertypeList struct {
	Ethertype int `json:"ethertype"` // ether type value
}

// UrwbRadio represents URWB radio configuration.
...
// UrwbChannelLists represents URWB channel list configuration.
type UrwbChannelLists struct {
	ChannelListEntry []UrwbChannelListEntry `json:"channel-list-entry"`
}

// UrwbChannelListEntry represents a single channel list entry.
