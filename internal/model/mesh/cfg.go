...
// MeshCfgData represents mesh configuration data container.
type MeshCfgData struct {
	Mesh         *MeshGlobalCfg        `json:"mesh,omitempty"`
	MeshProfiles MeshProfilesContainer `json:"mesh-profiles"`
}

// MeshGlobalCfg represents global mesh configuration.
type MeshGlobalCfg struct {
	// YANG 17.12.1 configuration fields
...
type MeshProfile struct {
	ProfileName string  `json:"profile-name"`          // Mesh profile identifier
	...
	// Additional fields requiring evidence verification
...
type MeshBackhaulDataRate struct {
	Type                   *string `json:"type,omitempty"`                      // Backhaul data rate type (YANG: IOS-XE 17.12.1+)
	Rate                   *string `json:"rate,omitempty"`                      // 802.11abg data rate (YANG: IOS-XE 17.12.1+)
	Dot11NMCSIndex         *uint8  `json:"dot11n-mcs-index,omitempty"`          // 802.11n MCS index (YANG: IOS-XE 17.12.1+)
	Dot11ACMCSIndex        *uint8  `json:"dot11ac-mcs-index,omitempty"`         // 802.11ac MCS index (YANG: IOS-XE 17.12.1+)
	SpatialStream          *uint8  `json:"spatial-stream,omitempty"`            // 802.11ac spatial stream (YANG: IOS-XE 17.12.1+)
	Dot11AXMCSIndex        *uint8  `json:"dot11ax-mcs-index,omitempty"`         // 802.11ax MCS index (YANG: IOS-XE 17.12.1+)
	Dot11AXSpatialStreamBG *uint8  `json:"dot11ax-spatial-stream-bg,omitempty"` // 802.11ax BG spatial stream (YANG: IOS-XE 17.12.1+)
	Dot11AXSpatialStreamA  *uint8  `json:"dot11ax-spatial-stream-a,omitempty"`  // 802.11ax A spatial stream (YANG: IOS-XE 17.12.1+)
}
